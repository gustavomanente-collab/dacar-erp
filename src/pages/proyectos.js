...
const SHEETS_CATALOGO_URL = 'https://docs.google.com/spreadsheets/d/1UCHkxdliJAkL5yT9bo_70V0idTgEo_5x_PLiBSxiLxM/edit'
              ...
              <div class="flex items-center justify-between text-xs">
                ...
                ${p.precio_venta_usd > 0 ? `
                  <div class="text-right">
                    ...
                    <span class="font-bold text-green-700">${fmtUsd(p.precio_venta_usd)}</span>
                  </div>
    ...
    <div class="bg-gradient-to-r from-gray-900 to-gray-700 text-white p-5 rounded-t-2xl -m-6 mb-4">
      <div class="flex items-start justify-between">
        <div>
          ...
          <h2 class="text-2xl font-black">${escapeHtml(proyecto.nombre)}</h2>
          <p class="text-sm text-gray-300 mt-1">
            ${escapeHtml(proyecto.clientes?.nombre || 'Sin cliente')}
            ${proyecto.clientes?.obra ? ' · ' + escapeHtml(proyecto.clientes.obra) : ''}
...
function bindEventosFicha(proyectoId, modal, data) {
  window.subTabFicha = (tab) => {
    ...
    if (tab === 'analisis')    { cont.innerHTML = renderAnalisisPrecios(data); bindAnalisisEvents(proyectoId, modal, data) }
          ...
          <table class="w-full text-xs">
            ...
            <tbody>
              ${lista.map((it, i) => {
                ...
                const subtotalUsd   = it.moneda === 'ARS' ? subtotalLinea / tc : subtotalLinea
                return `
...
function bindAnalisisEvents(proyectoId, modal, data) {
  ...
  window.actualizarProyecto = async (campo, valor) => {
    ...
    update.costo_total_usd  = r.costoTotal
    update.precio_venta_usd = r.precioVenta
    update.precio_venta_ars = r.precioVentaArs
    update.costo_total_ars  = r.costoTotalArs
  ...
  window.agregarItem = async (categoria) => {
    ...
    const m = crearModal(`
      ...
      <div class="bg-blue-50 border border-blue-200 rounded-lg p-3 mb-4">
        ...
        <input id="ai-busca" type="text" placeholder="🔍 Escribí para buscar..."
      ...
      <div class="space-y-3">
        <div>
          ...
          <input id="ai-desc" type="text" class="w-full rounded-lg border-gray-300 text-sm" />
        </div>
      ...
      dropResult.innerHTML = Object.entries(grupos).map(([sub, lista]) => `
        ...
        ${lista.map(it => `
          <div onclick="seleccionarItemCatalogo('${it.id}')"
            ...
            <div class="text-right">
              <p class="font-bold text-blue-700">${it.moneda === 'ARS' ? fmt(it.precio_unitario) : fmtUsd(it.precio_unitario)}</p>
  ...
  return `
    <div class="grid grid-cols-3 gap-4">
      <div class="bg-blue-50 border border-blue-200 rounded-xl p-4 text-center">
        ...
        <p class="text-2xl font-black text-blue-700">${fmtUsd(r.costoDirecto)}</p>
      </div>
      <div class="bg-orange-50 border border-orange-200 rounded-xl p-4 text-center">
        <p class="text-xs text-orange-600">Costo real</p>
        <p class="text-2xl font-black text-orange-700">U$S 0,00</p>
      </div>
      <div class="bg-gray-50 border border-gray-200 rounded-xl p-4 text-center">
        <p class="text-xs text-gray-600">Desvío</p>
        <p class="text-2xl font-black text-gray-700">--</p>
      </div>
    </div>
...
async function renderCatalogo() {
  ...
  el.innerHTML = `
    <div class="flex justify-between items-center mb-4">
      ...
      <div class="flex gap-2">
        <a href="${SHEETS_CATALOGO_URL}" target="_blank"
    ...
    ${(items||[]).length === 0 ? `
      <div class="bg-yellow-50 border border-yellow-200 rounded-xl p-6 text-center">
        <p class="text-yellow-700 font-semibold">📚 El catálogo está vacío</p>
        <p class="text-yellow-600 text-sm mt-1">Editá la planilla en Sheets y luego sincronizá, o agregá items manualmente.</p>
      </div>
                    ...
                    ${lista.map((it, i) => `
                      <tr class="cat-row border-t border-gray-100 ${i%2===0?'bg-white':'bg-gray-50'}"
                        ...
                        <td class="px-2 py-1.5 w-32 text-right">
                          <input type="number" step="0.01" value="${it.precio_unitario}"
                            class="w-full bg-transparent border-0 text-xs text-right"
                            onblur="actualizarCatItem('${it.id}','precio_unitario',this.value)" />
                        </td>
                        <td class="px-2 py-1.5 w-16 text-center">
                          <select class="bg-transparent border-0 text-xs"
                            onchange="actualizarCatItem('${it.id}','moneda',this.value)">
                            <option value="USD" ${it.moneda==='USD'?'selected':''}>U$S</option>
                            <option value="ARS" ${it.moneda==='ARS'?'selected':''}>$</option>
                          </select>
                        </td>
  ...
  window.actualizarCatItem = async (id, campo, valor) => {
    const v = (campo === 'precio_unitario') ? parseFloat(valor) || 0 : valor
    await supabase.from('catalogo_items').update({ [campo]: v }).eq('id', id)
  }
  ...
  window.abrirNuevoItemCatalogo = () => {
      ...
      <div class="space-y-3">
        ...
        <div class="grid grid-cols-2 gap-3">
          ...
          <div>
            ...
            <select id="ci-moneda" class="w-full rounded-lg border-gray-300 text-sm">
              <option value="USD">U$S</option>
              <option value="ARS">$ Pesos</option>
            </select>
          </div>
        ...
        <div class="grid grid-cols-2 gap-3">
          <div>
            ...
            <input id="ci-precio" type="number" step="0.01" value="0"
              class="w-full rounded-lg border-gray-300 text-sm text-right" />
          </div>
          <div>
    ...
    window.guardarItemCatalogo = async () => {
      ...
      await supabase.from('catalogo_items').insert({
        ...
        moneda: document.getElementById('ci-moneda').value,
        precio_unitario: precio,
